...
class UsersToProjects(db.Model):
    ...
    async def get_user_role_id_in_project(cls, user_id: int, project_id: int) -> Optional[int]:
        result: UsersToProjects = await cls.query.where((cls.project_id == project_id) & (cls.user_id == user_id))\
            .gino.first()
        if result:
...
class Project(db.Model):
    ...
    async def get_names_by_user_id(cls, user_id: int) -> List[Tuple[int, str]]:
        return await cls.join(UsersToProjects, cls.id == UsersToProjects.project_id). \
            join(User, User.id == UsersToProjects.user_id). \
            select().where(User.id == user_id). \
            with_only_columns((cls.id, cls.name)).gino.all()

    @classmethod
    async def get_token(cls, project_id: int) -> str:
        result: Tuple[str] = await cls.query.select().where(cls.id == project_id).\
            with_only_columns((cls.access_token, )).gino.first()
...
class User(db.Model):
    ...
    async def get_by_project_id(cls, project_id: int) -> List[Tuple[int, int, str]]:
        return await cls.join(UsersToProjects, UsersToProjects.user_id == cls.id).select().\
            where(UsersToProjects.project_id == project_id).with_only_columns((cls.id, UsersToProjects.role_id, cls.name)).gino.all()
