...
from application.telegram.menu.main.fsm import CreateProjectData, JoinToProjectData, make_redis_joining_key, \
...
async def projects_handler(query: CallbackQuery):
    ...
    projects_data = [ProjectsListRenderingInfo(**{
...
def make_create_project_handler(redis: Redis):
    async def set_create_project_state(query: CallbackQuery):
        parsed_query_data = query.data.split(":")
        user_id = int(parsed_query_data[1])
...
def make_create_project_callback(redis: Redis):
    async def create_project(message: Message):
        ...
        return_markup = return_button(InlineKeyboardMarkup(row_width=3), user_id)
        await message.answer(f"Project {project_name} was successfully created!\n"
                             f"You can now browse it in 'My projects'", reply_markup=return_markup)

    return create_project


def make_join_to_project_handler(redis: Redis):
...
def make_join_to_project_callback(redis: Redis):
    async def join_to_project(message: Message):
        token = message.text
        chat_id = message.chat.id
        ...
        already_joined = await UsersToProjects.query.where(
            (UsersToProjects.project_id == project.id) & (UsersToProjects.user_id == user_id)).gino.first()
        if already_joined:
