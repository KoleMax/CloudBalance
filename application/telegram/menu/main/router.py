...
from application.telegram.fsm import make_redis_joining_key, make_redis_creating_key
from .handlers import (
    projects_handler,
    settings_handler,
    make_join_to_project_handler,
    make_join_to_project_callback,
    make_create_project_handler,
    make_create_project_callback,
)
from .renderers import CALLBACK_DATA, RETURN_CALLBACK_DATA, MainMenuCommands
from .utils import make_return_handler


def configure_dispatcher(dispatcher: Dispatcher, redis: Redis, sync_redis: SyncRedis, db: Database) -> None:
    # redis fsm create project handler
    dispatcher.message_handler(
        lambda message: sync_redis.get(make_redis_creating_key(message.chat.id)) is not None
    )(make_create_project_callback(db, redis))
    # redis fsm join to project handler
    dispatcher.message_handler(
        lambda message: sync_redis.get(make_redis_joining_key(message.chat.id)) is not None
    ...
    dispatcher.callback_query_handler(CALLBACK_DATA.filter(command=MainMenuCommands.PROJECTS.name))(
        projects_handler
    )
    ...
    # settings callback
    dispatcher.callback_query_handler(CALLBACK_DATA.filter(command=MainMenuCommands.SETTINGS.name))(
        settings_handler
    )
