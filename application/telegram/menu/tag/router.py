...
from .fsm import make_redis_rename_tag_key
...
def configure_dispatcher(dispatcher: Dispatcher, redis: Redis, sync_redis: SyncRedis) -> None:
    ...
    dispatcher.message_handler(lambda message: sync_redis.get(make_redis_rename_tag_key(message.chat.id)) is not None) \
