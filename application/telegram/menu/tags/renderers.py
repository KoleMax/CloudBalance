...
CALLBACK_DATA = CallbackData('users_list', 'user_id', 'project_id', 'project_name',
                             'tag_id', 'tag_name', 'user_role_id')
...
class TagsListRenderer(ListRenderer):
    ...
    def add_callback(self, tag_data: TagInfo, ) -> str:
        return self.callback_data.new(user_id=self.user_id,
                                      project_id=self.project_id,
                                      project_name=self.project_name,
...
RETURN_CALLBACK_DATA = CallbackData('back_to_tags_list', 'current_user_id', 'project_id', 'project_name', 'command')
...
def return_button(markup: types.InlineKeyboardMarkup, user_id: int, project_id: int, project_name: str) -> types.InlineKeyboardMarkup:
    callback_data = RETURN_CALLBACK_DATA.new(current_user_id=user_id, project_id=project_id,
                                             project_name=project_name, command=RETURN_TO_PROJECT_LIST_COMMAND)
