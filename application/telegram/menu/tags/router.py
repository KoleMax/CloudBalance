...
from application.telegram.menu.tags.fsm import make_redis_create_tag_key
...
def configure_dispatcher(dispatcher: Dispatcher, redis: Redis, sync_redis: SyncRedis) -> None:
    ...
    dispatcher.message_handler(lambda message: sync_redis.get(make_redis_create_tag_key(message.chat.id)) is not None)\
    ...
    # TODO: enum
    dispatcher.callback_query_handler(CALLBACK_DATA.filter(user_role_id='1'))(tag_menu_handler)
    dispatcher.callback_query_handler(CALLBACK_DATA.filter(user_role_id='2'))(tag_menu_handler)
