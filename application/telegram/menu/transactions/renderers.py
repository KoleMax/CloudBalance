...
class TransactionsMenuRenderer(MenuRenderer):
    ...
    commands_with_permissions_to_text: Dict[Tuple[commands, Tuple], str] = {
        (TransactionsMenuCommands.ADD_INCOME, (UserRoles.USER.value, UserRoles.ADMIN.value, UserRoles.CREATOR.value)):
            'Add income',
        (TransactionsMenuCommands.ADD_EXPENSE, (UserRoles.USER.value, UserRoles.ADMIN.value, UserRoles.CREATOR.value)):
            'Add expense',
        (TransactionsMenuCommands.LIST_TRANSACTIONS, (UserRoles.ADMIN.value, UserRoles.CREATOR.value)):
            'List transactions',
    }
...
CHOSE_TAG_CALLBACK_DATA = CallbackData('chose_tag_menu', 'user_id', 'project_id', 'project_name',
                                       'transaction_type_id', 'tag_id', 'tag_name')
...
class ChoseTagListRenderer(ListRenderer):
    ...
    def add_callback(self, tag_data: TagInfo, ) -> str:
...
LIST_TRANSACTIONS_CALLBACK_DATA = CallbackData('transactions_list_menu', 'user_id', 'project_id', 'project_name',
                                               'transaction_id', )
...
class TransactionListRenderer(ListRenderer):
    ...
    def __call__(self, list_data: List[TransactionsListRenderingInfo], *args, **kwargs) -> types.InlineKeyboardMarkup:
        list_markup = super().__call__(list_data, *args, **kwargs)
        return return_button(list_markup, self.user_id, self.project_id, self.project_name)

    def add_callback(self, transaction_data: TransactionInfo, ) -> str:
        return self.callback_data.new(
            user_id=self.user_id,
            project_id=self.project_id,
            project_name=self.project_name,
...
LIST_TRANSACTIONS_RETURN_CALLBACK_DATA = CallbackData('back_to_transactions_list', 'user_id', 'project_id', 'project_name')


def list_transactions_return_button(markup: types.InlineKeyboardMarkup, user_id: int, project_id: int, project_name: str) -> types.InlineKeyboardMarkup:
    callback_data = LIST_TRANSACTIONS_RETURN_CALLBACK_DATA.new(user_id=user_id, project_id=project_id,
                                             project_name=project_name)
