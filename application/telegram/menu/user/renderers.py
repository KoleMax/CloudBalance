...
class UserMenuRenderer(MenuRenderer):
    ...
    commands_with_permissions_to_text: Dict[CommandToPermission, str] = {
        CommandToPermission(
            **{
                "command": UserMenuCommands.CHANGE_ROLE,
                "permissions": (UserRoles.ADMIN.value, UserRoles.CREATOR.value),
            }
        ): "Change role",
        CommandToPermission(
            **{"command": UserMenuCommands.KICK, "permissions": (UserRoles.CREATOR.value, UserRoles.ADMIN.value)}
        ): "Kick",
    }
    ...
    def __call__(self, *args, **kwargs) -> types.InlineKeyboardMarkup:
        buttons = [
            self.render_button(command_to_permission.command, text)
...
CHOSE_ROLE_CALLBACK_DATA = CallbackData(
    "chose_role_menu",
    "current_user_id",
    "current_user_role_id",
    "project_id",
