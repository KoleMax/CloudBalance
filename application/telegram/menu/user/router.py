...
from application.telegram.fsm import make_redis_kick_key
from .handlers import (
    make_return_handler,
    make_kick_handler,
    make_kick_callback,
    chose_role_handler,
    make_set_role_handler,
)
...
def configure_dispatcher(dispatcher: Dispatcher, bot: Bot, redis: Redis, sync_redis: SyncRedis) -> None:
    ...
    # change role
    dispatcher.callback_query_handler(CALLBACK_DATA.filter(command=UserMenuCommands.CHANGE_ROLE.name))(
        chose_role_handler
    )
    # set chosen role
    dispatcher.callback_query_handler(CHOSE_ROLE_CALLBACK_DATA.filter())(make_set_role_handler(bot))
    # kick user callback
    dispatcher.callback_query_handler(CALLBACK_DATA.filter(command=UserMenuCommands.KICK.name))(
        make_kick_handler(redis)
    )
    # return callback
