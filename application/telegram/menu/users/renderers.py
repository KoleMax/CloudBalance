...
CALLBACK_DATA = CallbackData(
    "users_list", "current_user_id", "current_user_role_id", "project_id", "project_name", "user_id", "user_name"
)
            ...
            if self.user_id == info.callback_data.id:
                buttons.append(
                    types.InlineKeyboardButton(
                        f"{info.button_title} (You)", callback_data=MOCK_CALLBACK_DATA.new(mock=True)
                    )
                )
                continue
            button_title = info.button_title

            if info.callback_data.role_id == enums.UserRoles.CREATOR.value:
                button_title += f" ({enums.UserRoles.CREATOR.name.capitalize()})"
            elif info.callback_data.role_id == enums.UserRoles.ADMIN.value:
                button_title += f" ({enums.UserRoles.ADMIN.name.capitalize()})"
            if self.user_role_id <= info.callback_data.role_id:
                buttons.append(self.render_button(button_title, info.callback_data))
            else:
                buttons.append(
                    types.InlineKeyboardButton(button_title, callback_data=MOCK_CALLBACK_DATA.new(mock=True))
                )
...
def return_button(
    ...
    callback_data = RETURN_CALLBACK_DATA.new(
        current_user_id=user_id, project_id=project_id, project_name=project_name
    )
